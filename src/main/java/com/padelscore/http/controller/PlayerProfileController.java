...
public class PlayerProfileController {
    ...
    @GetMapping
    @Operation(summary = "Получить все профили игроков", description = "Возвращает список всех профилей игроков")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Получить профиль игрока по Telegram ID", description = "Возвращает профиль игрока по Telegram ID")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Профиль игрока успешно получен",
                    content = @Content(schema = @Schema(implementation = PlayerProfileDto.class))),
        @ApiResponse(responseCode = "404", description = "Профиль игрока не найден")
    ...
    public ResponseEntity<PlayerProfileDto> getPlayerProfileByTelegramId(
            @Parameter(description = "Telegram ID игрока", required = true) @PathVariable Long telegramId) {
        PlayerProfileDto profile = playerProfileService.getPlayerProfileByTelegramId(telegramId);
        return ResponseEntity.ok(profile);
    }
    
    @GetMapping("/search")
    @Operation(summary = "Поиск профилей игроков", description = "Поиск профилей игроков по имени или фамилии")
    @ApiResponses(value = {
    ...
    public ResponseEntity<PlayerProfileDto> createPlayerProfile(@Valid @RequestBody CreatePlayerProfileRequest request) {
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Обновить профиль игрока", description = "Обновляет профиль игрока (изменения применятся ко всем командам)")
    @ApiResponses(value = {
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Удалить профиль игрока", description = "Удаляет профиль игрока (только если игрок не состоит ни в одной команде)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "204", description = "Профиль игрока успешно удален"),
        @ApiResponse(responseCode = "400", description = "Невозможно удалить: игрок состоит в командах"),
        @ApiResponse(responseCode = "404", description = "Профиль игрока не найден")
    })
    ...
    @GetMapping("/{id}/teams")
    @Operation(summary = "Получить команды игрока", description = "Возвращает список всех команд, в которых состоит игрок")
    @ApiResponses(value = {
