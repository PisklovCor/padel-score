...
@Tag(name = "Связи игрок-команда", description = "API для управления связями игроков с командами")
public class TeamPlayerController {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Получить связь игрок-команда", description = "Возвращает информацию о связи игрока с командой")
    @ApiResponses(value = {
    ...
    @PostMapping
    @Operation(summary = "Добавить игрока в команду", description = "Создает связь между существующим профилем игрока и командой")
    @ApiResponses(value = {
    ...
    public ResponseEntity<TeamPlayerDto> addPlayerToTeam(@RequestBody AddPlayerToTeamRequest request) {
    ...
    public ResponseEntity<TeamPlayerDto> createPlayer(@Valid @RequestBody CreatePlayerRequest request) {
    ...
    @Operation(summary = "Обновить позицию игрока в команде", description = "Обновляет только позицию игрока в конкретной команде")
    @ApiResponses(value = {
    ...
    public ResponseEntity<TeamPlayerDto> updatePlayer(
            @Parameter(description = "ID связи игрока с командой (TeamPlayer)", required = true) @PathVariable Integer id,
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Удалить связь игрок-команда", description = "Удаляет связь игрока с командой (профиль игрока сохраняется)")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Удалить игрока из команды", description = "Удаляет связь игрока с конкретной командой")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Void> removePlayerFromTeam(
            @Parameter(description = "ID команды", required = true) @PathVariable Integer teamId,
            @Parameter(description = "ID профиля игрока", required = true) @PathVariable Integer playerProfileId) {
