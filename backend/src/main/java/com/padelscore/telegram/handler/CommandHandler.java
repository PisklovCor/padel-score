...
public class CommandHandler {
  ...
  /**
   * Находит подходящий обработчик команды по тексту сообщения и выполняет его; при отсутствии
   * совпадения отправляет сообщение об неизвестной команде.
   */
...
//            }
  ...
  private void handleCreateTournament(Long chatId, Long userId, TelegramLongPollingBot bot,
      String commandText) {
  ...
  private void handleAddTeam(Long chatId, Long userId, TelegramLongPollingBot bot,
      String commandText) {
  ...
  private void handleAddPlayer(Long chatId, Long userId, TelegramLongPollingBot bot,
      String commandText) {
    String[] parts = commandText.split(" ", 3);
    if (parts.length < 3 || parts[2].trim().isEmpty()) {
    ...
    try {
      ...
      TeamPlayerDto player = teamPlayerService.createPlayer(teamId, firstName, lastName, userId,
          null, "primary");
  ...
  private void handleAddMatch(Long chatId, Long userId, TelegramLongPollingBot bot,
      String commandText) {
