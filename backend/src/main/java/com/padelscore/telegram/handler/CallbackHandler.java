...
public class CallbackHandler {
  ...
  /**
   * Находит обработчик по callback data нажатой кнопки, выполняет его и отвечает на callback. При
...
//            }
  ...
  private void handleTeamCallback(String data, Long chatId, Integer messageId, Long userId,
      TelegramLongPollingBot bot) throws TelegramApiException {
  ...
  private void handlePlayersList(String data, Long chatId, Integer messageId,
      TelegramLongPollingBot bot) throws TelegramApiException {
    ...
    if (players.isEmpty()) {
      ...
      StringBuilder text = new StringBuilder("👤 Игроки команды ").append(team.getName())
          .append(":\n\n");
  ...
  private void handlePlayerCallback(String data, Long chatId, Integer messageId,
      TelegramLongPollingBot bot) throws TelegramApiException {
  ...
  private void handlePlayerCreate(String data, Long chatId, Long userId,
  ...
  private void handleMatchesList(String data, Long chatId, Integer messageId,
      TelegramLongPollingBot bot) throws TelegramApiException {
    Integer tournamentId = Integer.parseInt(data.split("_")[2]);
  ...
  private void handleMatchCallback(String data, Long chatId, Integer messageId, Long userId,
      TelegramLongPollingBot bot) throws TelegramApiException {
    ...
    message.setReplyMarkup(
        keyboardUtil.getMatchMenu(matchId, match.getTournamentId(), match.getStatus()));
    bot.execute(message);
  }

  private void handleMatchCreate(String data, Long chatId, Long userId,
      TelegramLongPollingBot bot) {
    Integer tournamentId = Integer.parseInt(data.split("_")[2]);
  ...
  private void handleMatchResultInput(String data, Long chatId, Integer messageId, Long userId,
      TelegramLongPollingBot bot) throws TelegramApiException {
  ...
  private void handleQuickResult(String data, Long chatId, Integer messageId, Long userId,
      TelegramLongPollingBot bot) throws TelegramApiException {
  ...
  private void handleMatchView(String data, Long chatId, Integer messageId,
      TelegramLongPollingBot bot) throws TelegramApiException {
    Integer matchId = Integer.parseInt(data.split("_")[2]);
    MatchDto match = matchService.getMatch(matchId);
  ...
  private void handleMatchDispute(String data, Long chatId, Integer messageId, Long userId,
      TelegramLongPollingBot bot) throws TelegramApiException {
    Integer matchId = Integer.parseInt(data.split("_")[2]);
  ...
  private void handleLeaderboard(String data, Long chatId, Integer messageId,
      TelegramLongPollingBot bot) throws TelegramApiException {
    Integer tournamentId = Integer.parseInt(data.split("_")[1]);
    ...
    for (LeaderboardEntryDto entry : leaderboard) {
      text.append(String.format("%-3d %-20s %-4d %-4d %-4d %-8s %-6d\n",
          position++,
          entry.getTeamName().length() > 20 ? entry.getTeamName().substring(0, 17) + "..."
              : entry.getTeamName(),
          entry.getMatches(),
          entry.getWins(),
          entry.getLosses(),
  ...
  private void handleMainMenu(Long chatId, Integer messageId, TelegramLongPollingBot bot)
